...
struct TunePair FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  bool KeyCompareLessThan(const TunePair * const o) const {
    return *key() < *o->key();
  }
...
struct TuneCacheT : public ::flatbuffers::NativeTable {
  ...
  TuneCacheT() = default;
  TuneCacheT(const TuneCacheT &o);
  TuneCacheT(TuneCacheT&&) FLATBUFFERS_NOEXCEPT = default;
  TuneCacheT &operator=(TuneCacheT o) FLATBUFFERS_NOEXCEPT;
};
...
inline TunePairT *TunePair::UnPack(const ::flatbuffers::resolver_function_t *_resolver) const {
  auto _o = std::unique_ptr<TunePairT>(new TunePairT());
...
inline bool operator==(const TuneCacheT &lhs, const TuneCacheT &rhs) {
  return
      (lhs.tune_map.size() == rhs.tune_map.size() && std::equal(lhs.tune_map.cbegin(), lhs.tune_map.cend(), rhs.tune_map.cbegin(), [](std::unique_ptr<paddle::lite::fbs::opencl::proto::TuneCache_::TunePairT> const &a, std::unique_ptr<paddle::lite::fbs::opencl::proto::TuneCache_::TunePairT> const &b) { return (a == b) || (a && b && *a == *b); }));
}
...
inline TuneCacheT::TuneCacheT(const TuneCacheT &o) {
  tune_map.reserve(o.tune_map.size());
  for (const auto &tune_map_ : o.tune_map) { tune_map.emplace_back((tune_map_) ? new paddle::lite::fbs::opencl::proto::TuneCache_::TunePairT(*tune_map_) : nullptr); }
}

inline TuneCacheT &TuneCacheT::operator=(TuneCacheT o) FLATBUFFERS_NOEXCEPT {
  std::swap(tune_map, o.tune_map);
  return *this;
}

inline TuneCacheT *TuneCache::UnPack(const ::flatbuffers::resolver_function_t *_resolver) const {
  auto _o = std::unique_ptr<TuneCacheT>(new TuneCacheT());
  UnPackTo(_o.get(), _resolver);
  return _o.release();
}
...
inline void TuneCache::UnPackTo(TuneCacheT *_o, const ::flatbuffers::resolver_function_t *_resolver) const {
  (void)_o;
  (void)_resolver;
  { auto _e = tune_map(); if (_e) { _o->tune_map.resize(_e->size()); for (::flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { if(_o->tune_map[_i]) { _e->Get(_i)->UnPackTo(_o->tune_map[_i].get(), _resolver); } else { _o->tune_map[_i] = std::unique_ptr<paddle::lite::fbs::opencl::proto::TuneCache_::TunePairT>(_e->Get(_i)->UnPack(_resolver)); } } } else { _o->tune_map.resize(0); } }
}
...
inline paddle::lite::fbs::opencl::proto::TuneCache *GetMutableSizePrefixedTuneCache(void *buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<paddle::lite::fbs::opencl::proto::TuneCache>(buf);
}
